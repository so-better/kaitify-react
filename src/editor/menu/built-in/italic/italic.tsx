...
export default function ItalicMenu(props: ItalicMenuPropsType) {
  const { state } = useWrapperContext()

  //是否激活
  const isActive = useMemo(() => {
    return state.editor.value?.commands.isItalic?.() ?? false
  }, [state.editor])
  //是否禁用
  ...
  const onOperate = () => {
    if (isActive) {
      state.editor.value?.commands.unsetItalic?.()
    } else {
      state.editor.value?.commands.setItalic?.()
    }
  }

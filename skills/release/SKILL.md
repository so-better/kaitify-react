---
name: release
description: kaitify-react 发布流程，按步骤完成版本号更新、源码打包、文档更新、changelog 更新、文档打包、README 更新、npm 发布，每步均需用户确认后执行。
---
...
- `src/editor/wrapper/props.ts`：Wrapper 组件的 props 类型定义
- `src/editor/wrapper/wrapper.tsx`：Wrapper 组件实现（含 emits、slots 等）
- `src/editor/bubble/props.ts`：Bubble 组件的 props 类型定义
- `src/editor/bubble/bubble.tsx`：Bubble 组件实现
- `src/editor/menu/props.ts`：Menu 组件的 props 类型定义
- `src/editor/menu/built-in/` 下各内置菜单的 `props.ts` 和 `index.ts`：各菜单组件的 props 类型
- `src/core/` 下各核心组件的 `props.ts`：Button、Checkbox、Tabs、Icon、Popover、Teleport、Divider 的 props 类型
- `src/hooks/use-editor.ts`：useEditor hook 的参数和返回值
...
- `docs/guide/wrapper.md`：Wrapper 组件文档
- `docs/guide/bubble.md`：Bubble 组件文档
- `docs/guide/menu.md`：Menu 组件文档
- `docs/guide/shortcut.md`：快捷键文档
- `docs/guide/locale.md`：国际化文档
- `docs/guide/icon.md`：Icon 组件文档
- `docs/guide/button.md`（若存在）：Button 组件文档
- `docs/guide/checkbox.md`：Checkbox 组件文档
- `docs/guide/tabs.md`：Tabs 组件文档
- `docs/guide/divider.md`：Divider 组件文档
- `docs/menus/introduction.md`：内置菜单介绍
- `docs/menus/built-in/` 下各内置菜单文档：核对每个菜单组件的 props 名称及类型

重点核查：

- 组件名称是否与源码导出名一致
- Props 名称、类型、默认值是否与 TypeScript 类型定义完全一致，不得凭推断或记忆填写
- 代码示例中的用法是否符合真实的组件接口
- 是否存在拼写错误或注释错误
...
用户确认后，**重新读取** `src/index.ts`、`src/editor/wrapper/props.ts`、`src/editor/bubble/props.ts`、`src/editor/menu/props.ts`、`src/hooks/use-editor.ts`、`src/locale/index.ts`、`package.json` 以及更新后的 `docs/` 文档，对 `README.md` 进行核查和修正：
...
2. **简介**：根据 `docs/guide/introduction.md` 提炼，说明这是什么、解决什么问题、技术栈（React + TipTap）
...
7. **useEditor Hook**：说明 `useEditor` 的参数和返回值（取自 `src/hooks/use-editor.ts` 和对应文档）
8. **内置菜单**：列出所有内置菜单组件名称（从 `src/editor/menu/built-in/` 导出列表），说明可直接使用
9. **国际化**：说明支持的语言（取自 `src/locale/index.ts` 的 `LocaleType`），附代码示例
10. **文档链接**：完整文档地址
11. **License**
